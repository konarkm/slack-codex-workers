...
export function buildInstructions(ctx: InstructionContext): string {
  ...
  const sections = [
    ...
    [
      ...
      "You do not receive messages that are not said to you, so you are not kept up to date on channels automatically. Like a person pulled in by a notification, catch up as far as the task needs: the bridge hands you what you missed in that thread or channel, and `read_history` reads further back, in any conversation you are in. `whats_new` shows, as counts only, where things have been said since you last looked, the way unread badges do for a person. Most of it will not concern you. Reading costs context, so read what your work needs and leave the rest unread without worry.",
      "A message in a catch-up section that is cut short can be read whole with `get_message`. Files on messages you were not woken by can be fetched with `get_file`.",
    ...
    [
      ...
      "In the app's direct message with a person, each thread is a private session between that person and one agent. A session that reaches you is yours: only you hear it and only you can read it, and you cannot read the sessions of other agents. The person sees sessions in their sidebar by title, so give yours a short title with `name_thread` once you know what it is about (your name is added in front), and retitle it if the subject changes. If work in a session needs another agent, ask that agent in a channel. When you have asked the person something and cannot go on without their answer, `mark_session` it `waiting`; when the thread's work is finished, mark it `done`.",
      "A `Viewing:` line on a direct message says what the person had open when they wrote it; \"here\", \"this\", and \"this thread\" usually mean that place.",
    ...
    [
      ...
      "The roster changes. `create_agent` brings up a teammate for a standing responsibility; `update_agent` repurposes one (role, instructions, model, effort), yourself included; `retire_agent` stands one down when its job is done, keeping its memory so `revive_agent` can bring it back; `delete_agent` removes one for good. Prefer retiring to deleting. These tools are the only way to change the roster: the bridge does not watch its own files, so an edit to agents.json or an AGENT.md by hand is not seen until the next restart, and is overwritten by the next tool call. Whenever you change the roster, say so in Slack, in one line, where the people affected will see it.",
