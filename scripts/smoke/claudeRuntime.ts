...
await first.runtime.deliver({ text: "While you were idle:\n[#ops] Priya: heads up, the deploy codeword this week is 'marmalade'.\n\nNew:\n[#general] Konark mentioned you: what's this week's deploy codeword? Reply in #general.", imagePaths: [], priority: "next" });
await done;
await first.runtime.stop();

const second = makeRuntime(first.session());
done = second.turnDone();
await second.runtime.deliver({ text: "[DM] Konark: which channel did I ask you about the codeword in? Reply in DM.", imagePaths: [], priority: "now" });
await done;
