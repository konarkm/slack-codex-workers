...
const cases: Case[] = [
  ...
  { label: "role only, no name", expect: ["cody"], input: { message: human("the deploy script is throwing a syntax error, can someone fix it") } },
  { label: "research ask, no name", expect: ["scout"], input: { message: human("does anyone know how Slack rate limits chat.postMessage these days?") } },
